...
run(function()
    ...
    ManualHeader = vape.Categories.realista:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not ActionRemote then
                    notif('ManualHeader', 'Remote não encontrado', 3, 'warning')
                    task.defer(function()
                        if ManualHeader.Enabled then ManualHeader:Toggle() end
                    end)
                    return
                end
                ...
                -- ✅ SEQUÊNCIA EXATA DO JOGO
                local originalPower = hum.JumpPower

                -- 1. Força JumpPower pra 35 (valor real do jogo)
                hum.JumpPower = JumpPower.Value

                -- 2. Muda estado pra Jumping
                pcall(function()
                ...
                -- 4. Decide entre Header ou BicycleKick baseado na altura da bola
                local ball = findBall()
                local action = 'Header'

                if ball and ball.Position.Y >= 10 then
                    action = 'BicycleKick'
                end

                ActionRemote:FireServer(action)

                -- 5. Espera 0.2s e restaura (igual o jogo faz)
                ...
                notif('ManualHeader', action .. ' disparado', 1)
        ...
        Tooltip = 'Header/BicycleKick igual apertar espaço. JumpPower 35 = valor real do jogo.'
    })

    JumpPower = ManualHeader:CreateSlider({
        Name = 'Jump Power',
        Min = 20, Max = 100, Default = 35,
        Tooltip = '35 = valor real do jogo (descoberto no LocalScript)'
    })
end)
...
run(function()
    ...
    local TACKLE_DASH_SPEED = 35      -- velocity acima disso = provavelmente tackle
    local TACKLE_COOLDOWN = 0.8        -- não dribla 2x em menos de 0.8s

    -- ✅ Caches
    local cachedBall = nil
    local ballConnections = {}
    local iHaveBall = false
    local lastDribbleTime = 0

    -- ✅ Cache de inimigos (atualizado em PlayerAdded/CharacterAdded)
    local enemyCache = {}  -- [player] = { root, bools, tackling, lastVel, hasTackleAnim, animSince }
    ...
    local function rebuildSelfCache()
        ...
        local localBool = char:FindFirstChild("LocalBool")
        ...
        charCache.powerShoot = localBool and localBool:FindFirstChild("PowerShoot")
    ...
    local function canDribble()
        ...
        if charCache.powerShoot and charCache.powerShoot.Value then return false end
    ...
    local function doDribble(reason)
        ...
        notif('AutoDribble', '🛡️ ' .. (reason or 'Drible'), 1)
    end

    -- ✅ Setup do cache de inimigo
    local function cacheEnemy(plr)
        ...
        local function setup(char)
            ...
            enemyCache[plr] = {
                ...
                root = root,
                bools = bools,
                tackling = bools:FindFirstChild("Tackling"),
                animSince = 0,
                hasTackleAnim = false,
            }

            -- Listener de animação
            hum.AnimationPlayed:Connect(function(track)
                if not AutoDribble.Enabled then return end
                local animId = track.Animation and track.Animation.AnimationId
                if animId and animId:find(TACKLE_ANIM_ID) then
                    local cache = enemyCache[plr]
                    if cache then
                        cache.hasTackleAnim = true
    ...
    local function setupBallWatcher()
        ...
        local existingWeld = ball:FindFirstChild("playerWeld")
        if existingWeld and existingWeld:IsA("Weld") and existingWeld.Part0 then
            iHaveBall = existingWeld.Part0.Parent == lplr.Character
        else
            iHaveBall = false
        end
        ...
        table.insert(ballConnections, ball.ChildAdded:Connect(function(child)
            if child.Name == "playerWeld" and child:IsA("Weld") then
                ...
                if rootPart and rootPart.Parent == lplr.Character then
                    iHaveBall = true
                    -- ✅ GRAB PROTECTION: peguei a bola, checa ameaças imediatas
                    task.defer(function()
                        ...
                        for plr, cache in enemyCache do
                            if plr.Team ~= lplr.Team and cache.tackling and cache.tackling.Value then
                                local dist = (cache.root.Position - myRoot.Position).Magnitude
                                if dist <= DangerRadius.Value then
                                    doDribble('Grab Protection')
                                    return
                                end
                            end
                        end
    ...
    -- ✅ Threat Score: calcula quão perigoso é o inimigo (0-100)
    local function calculateThreat(cache, myRoot)
        ...
        -- Fora do range total
        if dist > DangerRadius.Value then return 0 end

        local score = 0

        -- ✅ Componente 1: Distância (mais perto = mais perigoso)
        -- 100 pts se tá colado, 0 pts se tá no limite
        local distScore = (1 - (dist / DangerRadius.Value)) * 100
        score = score + distScore * 0.4

        -- ✅ Componente 2: Tackling state (server-confirmed)
        if cache.tackling and cache.tackling.Value then
            score = score + 50  -- estado oficial de tackle
        end

        -- ✅ Componente 3: Velocidade na minha direção
        local vel = root.AssemblyLinearVelocity
        local horizVel = Vector3.new(vel.X, 0, vel.Z)
        local speed = horizVel.Magnitude

        if speed > TACKLE_DASH_SPEED then
            local dirToMe = (myRoot.Position - root.Position).Unit
            dirToMe = Vector3.new(dirToMe.X, 0, dirToMe.Z).Unit
            local velDir = horizVel.Unit
            local dot = velDir:Dot(dirToMe)

            if dot > 0.5 then
                -- Vindo na minha direção em alta velocidade
                local velScore = math.clamp((speed - TACKLE_DASH_SPEED) / 15, 0, 1) * 40 * dot
                score = score + velScore
            end
        end

        -- ✅ Componente 4: Anim recente de tackle
        local timeSinceAnim = tick() - cache.animSince
        if cache.hasTackleAnim and timeSinceAnim < 1.5 then
            score = score + 30 * (1 - timeSinceAnim / 1.5)
        end

        -- ✅ Componente 5: Ângulo de look
        local enemyLook = root.CFrame.LookVector
        local dirToMe = (myRoot.Position - root.Position).Unit
        enemyLook = Vector3.new(enemyLook.X, 0, enemyLook.Z).Unit
        dirToMe = Vector3.new(dirToMe.X, 0, dirToMe.Z).Unit
        local lookDot = enemyLook:Dot(dirToMe)

        if lookDot > 0.5 then
            score = score + 20 * lookDot
        end

        -- ✅ Bonus: Safe Radius (colado em mim, ameaça máxima)
        if dist <= SafeRadius.Value then
            score = math.max(score, ThreatThreshold.Value + 10)  -- sempre acima do threshold
        end
    ...
    AutoDribble = vape.Categories.realista:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not ActionRemote then
                    notif('AutoDribble', 'Remote Action não encontrado.', 5, 'warning')
                    AutoDribble:Toggle()
                    return
                end
                ...
                -- Cache de todos os players
                enemyCache = {}
                for _, plr in game.Players:GetPlayers() do
                    cacheEnemy(plr)
                end

                AutoDribble:Clean(game.Players.PlayerAdded:Connect(cacheEnemy))

                AutoDribble:Clean(game.Players.PlayerRemoving:Connect(function(plr)
                    enemyCache[plr] = nil
                end))
                ...
                -- ✅ HOT LOOP: Heartbeat avalia ameaças constantemente
                AutoDribble:Clean(runService.Heartbeat:Connect(function()
                    if not iHaveBall then return end
                    ...
                    local threatReason = nil

                    for plr, cache in enemyCache do
                        if plr.Team ~= lplr.Team and cache.root and cache.root.Parent then
                            local threat = calculateThreat(cache, myRoot)
                            if threat > highestThreat then
                                highestThreat = threat
                                local dist = (cache.root.Position - myRoot.Position).Magnitude
                                threatReason = string.format('%s (T:%d D:%d)', plr.Name, math.floor(threat), math.floor(dist))
                            end
                        end
                    end

                    if highestThreat >= ThreatThreshold.Value then
                        doDribble(threatReason)
                    end
                end))
            else
                for _, conn in ballConnections do
        ...
        Tooltip = 'AutoDribble com Threat Score System: detecta tackle por anim, velocity, proximidade e estado.'
    })
    ...
    SafeRadius = AutoDribble:CreateSlider({
        ...
        Tooltip = 'Dentro desse raio, ameaça é SEMPRE máxima (dribla)'
    })
    ...
    DangerRadius = AutoDribble:CreateSlider({
        ...
        Suffix = function(val) return val == 1 and 'stud' or 'studs' end,
        Tooltip = 'Distância máxima para avaliar ameaças'
    })

    ThreatThreshold = AutoDribble:CreateSlider({
        Name = 'Threat Threshold',
        Min = 30, Max = 100, Default = 70,
        Tooltip = 'Score mínimo de ameaça pra disparar drible (menor = mais sensível)'
    })
end)
...
run(function()
    ...
    local HEADER_ANIM_ID = '14685442782'
    local TACKLE_ANIM_ID = '14317040670'
    ...
    local charConn = nil
    local animConn = nil
    local weightConn = nil
    local headerChangedConn = nil
    local proactiveConn = nil
    ...
    local shutdownTaskId = 0  -- ✅ NOVO: ID pra invalidar tasks antigas

    local buffExpiresAt = 0
    local BUFF_DURATION_ESTIMATE = 1.0
    local PROACTIVE_MARGIN = 0.15

    local indexHookInstalled = false
    local oldIndex = nil

    local function getRealHeaderValue()
        if not headerBool or not oldIndex then return false end
        local ok, val = pcall(oldIndex, headerBool, 'Value')
    ...
    local function rebuildCharCache()
        ...
        cachedAnimator = cachedHum and cachedHum:FindFirstChildOfClass('Animator')
        cachedRoot = cachedChar:FindFirstChild('HumanoidRootPart')
        cachedBools = cachedChar:FindFirstChild('Bools')
        headerBool = cachedBools and cachedBools:FindFirstChild('Header')
    ...
    local function isHeaderAnim(animId)
        if not animId then return false end
        return tostring(animId):match('%d+') == HEADER_ANIM_ID
    end

    local function isTackleAnim(animId)
        if not animId then return false end
        return tostring(animId):match('%d+') == TACKLE_ANIM_ID
    end

    local function shouldNeverBoost(animId)
        if not animId then return false end
        local numId = tostring(animId):match('%d+')
        return NEVER_BOOST[numId] ~= nil
    end

    local function isActionPriority(track)
        local prio = track.Priority
        return prio == Enum.AnimationPriority.Action3 
    ...
    local function shouldBeActive()
        ...
        if not cachedHum or cachedHum.Health <= 0 then return false end
        ...
        if ReactiveMode.Enabled then
            if tick() > tackleDetectedUntil then return false end
        else
    ...
    local function watchEnemyTackles(plr)
        ...
        local function setup(char)
            ...
            local conn = hum.AnimationPlayed:Connect(function(track)
                ...
                local animId = track.Animation and track.Animation.AnimationId
                if not isTackleAnim(animId) then return end
    ...
    local function canBoost(track)
        if not track or not track.Animation then return false end
        local animId = track.Animation.AnimationId
        if isHeaderAnim(animId) then return false end
        if shouldNeverBoost(animId) then return false end
        if isActionPriority(track) then return false end
        return true
    end

    local function boostTrack(track)
        if not canBoost(track) then return end

        pcall(function()
            track:AdjustWeight(BoostWeight.Value, 0)
            if not trackStates[track] then
                trackStates[track] = { boosted = true }
            else
                trackStates[track].boosted = true
            end
        end)
    ...
    local function unboostTrack(track)
        if not track then return end
        pcall(function()
            track:AdjustWeight(1, 0)
        end)
        if trackStates[track] then
            trackStates[track].boosted = false
        end
    end

    local function killHeaderTrack(track)
        pcall(function()
            if not trackStates[track] then
                trackStates[track] = {
                    boosted = false,
                    originalPriority = track.Priority
                }
            end
    ...
    local function restoreHeaderTrack(track)
        pcall(function()
            local state = trackStates[track]
            if state and state.originalPriority then
                track.Priority = state.originalPriority
            end
            track:AdjustWeight(1, 0)
        end)
    end
    ...
    local function setupAnimBlock()
        ...
        if weightConn then weightConn:Disconnect() end

        if not cachedHum or not cachedAnimator then return end

        animConn = cachedHum.AnimationPlayed:Connect(function(track)
            if not HeaderShield.Enabled and not shuttingDown then return end

            local anim = track.Animation
            if not anim then return end

            if isHeaderAnim(anim.AnimationId) then
                killHeaderTrack(track)
            else
                if HeaderShield.Enabled and shouldBeActive() then
                    boostTrack(track)
                end
            end
        end)

        weightConn = runService.Heartbeat:Connect(function()
            if not HeaderShield.Enabled and not shuttingDown then return end
            if not cachedAnimator then return end

            local bw = BoostWeight.Value
            local bwThreshold = bw * 0.9

            for _, track in cachedAnimator:GetPlayingAnimationTracks() do
                pcall(function()
                    local anim = track.Animation
                    if not anim then return end
                    local animId = anim.AnimationId

                    if isHeaderAnim(animId) then
                        ...
                        if not canBoost(track) then
                            if track.WeightCurrent > 1.5 then
                                unboostTrack(track)
                            end
                            return
                        end

                        if shieldingActive then
                            if track.WeightCurrent < bwThreshold then
                                track:AdjustWeight(bw, 0)
                                trackStates[track] = trackStates[track] or {}
                                trackStates[track].boosted = true
                            end
                        else
                            if track.WeightCurrent > 1.5 then
            ...
            for track, _ in pairs(trackStates) do
    ...
    local function installShootBypass()
        ...
        if not hookmetamethod then
            notif('HeaderShield', 'Sem hookmetamethod!', 5, 'warning')
            return
        end
    ...
    local function setupProactiveRefresh()
        if proactiveConn then proactiveConn:Disconnect() end

        proactiveConn = runService.Heartbeat:Connect(function()
            if not HeaderShield.Enabled then return end
            if not shieldingActive then return end
            if not headerBool then return end

            local realValue = getRealHeaderValue()
            if not realValue then return end

            local timeLeft = buffExpiresAt - tick()
            if timeLeft <= PROACTIVE_MARGIN then
    ...
    HeaderShield = vape.Categories.realista:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not ActionRemote then
                    notif('HeaderShield', 'Remote não encontrado', 5, 'warning')
                    HeaderShield:Toggle()
                    return
                end
                ...
                -- ✅ NOVO: Invalida qualquer task de shutdown pendente
                shutdownTaskId = shutdownTaskId + 1
                shuttingDown = false
                ...
                HeaderShield:Clean(game.Players.PlayerAdded:Connect(function(plr)
                    rebuildPlayerCache()
                    watchEnemyTackles(plr)
                end))

                HeaderShield:Clean(game.Players.PlayerRemoving:Connect(function(plr)
                ...
                mainConn = runService.Heartbeat:Connect(function()
                    ...
                    if not cachedChar or not cachedChar.Parent 
                    ...
                    local realValue = getRealHeaderValue()
                    if realValue then return end

                    if tick() - lastFire < 0.03 then return end

                    fireHeader()
                end)

                notif('HeaderShield', '🛡️ Ativado (Hook + Spoof)', 3)
            else
                ...
                if proactiveConn then proactiveConn:Disconnect(); proactiveConn = nil end

                for plr, conn in pairs(enemyWatchers) do
                    conn:Disconnect()
                end
                enemyWatchers = {}
                ...
                if cachedAnimator then
                    for _, track in cachedAnimator:GetPlayingAnimationTracks() do
                        pcall(function()
                            ...
                            if not isHeaderAnim(anim.AnimationId) then
                ...
                -- ✅ NOVO: Captura ID atual
                shutdownTaskId = shutdownTaskId + 1
                local myTaskId = shutdownTaskId

                task.delay(1.5, function()
                    -- ✅ Se o ID mudou (módulo religou) OU tá ativo, não faz cleanup
                    if myTaskId ~= shutdownTaskId then return end
                    if HeaderShield.Enabled then return end

                    shuttingDown = false

                    if animConn then animConn:Disconnect(); animConn = nil end
                    if weightConn then weightConn:Disconnect(); weightConn = nil end
                ...
                shieldingActive = false
                buffExpiresAt = 0
                notif('HeaderShield', 'Desativado', 2)
            end
        end,
        Tooltip = 'Header shield com Hook + Spoof (zero delay, sem detecção M1)'
    })

    BoostWeight = HeaderShield:CreateSlider({
        Name = 'Boost Weight',
        Min = 100, Max = 100000, Default = 10000,
        Tooltip = 'Peso das outras anims. Maior = mais domínio sobre a header.'
    })

    OnlyWithBall = HeaderShield:CreateToggle({
        Name = 'Only With Ball',
        Default = true,
        Tooltip = 'Só ativa Header quando você está com a bola (via playerWeld)'
    })

    ReactiveMode = HeaderShield:CreateToggle({
        Name = 'Reactive Mode',
        Default = false,
        Tooltip = 'Só ativa Header quando detectar tackle inimigo'
    })

    ShootBypass = HeaderShield:CreateToggle({
        Name = 'Shoot Bypass',
        Default = true,
        Tooltip = 'Hook + Spoof: jogo pensa que Header é false, buff real fica no servidor'
    })
    ...
    AutoActivate = HeaderShield:CreateToggle({
        ...
        Tooltip = 'Só quando inimigo tá perto (ignorado se Reactive Mode estiver ativo)'
    })
...
run(function()
    ...
    BallTP = vape.Categories.realista:CreateModule({
        Name = 'BallTP',
        Function = function(callback)
            if callback then
                local ball = findBall()
                ...
                if not ball then
                    notif('BallTP', 'Ball not found in workspace.', 5, 'warning')
                    BallTP:Toggle()
                    return
                end
                ...
                if MovementMode.Value ~= 'Lerp' then
                    ...
                    repeat
                        if entitylib.isAlive then
                            -- Atualiza posição da ball em tempo real durante o lerp
                            ball = findBall()
                            if ball then
                        ...
                        elseif BallTP.Enabled then
                            BallTP:Toggle()
                            notif('BallTP', 'Character missing', 5, 'warning')
                        end
...
run(function()
    ...
    SwitchTeam = vape.Categories.realista:CreateModule({
        Name = 'SwitchTeam',
        Function = function(callback)
            if callback then
                local myTeam = lplr.Team
                ...
                if not myTeam or myTeam.Name:lower() == 'fans' then
                    notif('SwitchTeam', 'Você precisa estar em um time primeiro.', 5, 'warning')
                    SwitchTeam:Toggle()
                    return
                end
                ...
                if not enemy then
                    notif('SwitchTeam', 'Time inimigo não encontrado.', 5, 'warning')
                    SwitchTeam:Toggle()
                    return
                end

                TeamChangeRemote:FireServer(enemy.TeamColor, RoleMode.Value)
                notif('SwitchTeam', 'Trocando para ' .. enemy.Name .. ' (' .. RoleMode.Value .. ')', 3)
...
run(function()
    ...
    -- ⚠️ ATENÇÃO: O nome tem letra "I" maiúscula, NÃO "l" minúscula!
    -- ShootTheBaII (correto) vs ShootTheBall (HONEYPOT - banimento!)
    local ShootRemote = game:GetService('ReplicatedStorage').Remotes:FindFirstChild('ShootTheBaII')

    if not ShootRemote then
        warn('[AutoKick] Remote ShootTheBaII não encontrado!')
        return
    end
    ...
    AutoKick = vape.Categories.realista:CreateModule({
        ...
        Tooltip = 'Chuta a bola na direção que você olha.'
    })
...
run(function()
    ...
    local function isGameForce(child)
        ...
        return child:IsA("BodyVelocity") or child:IsA("LinearVelocity") or child:IsA("BodyGyro")
    end

    -- ✅ Calcula o target atual baseado no modo + posse
    ...
    -- ✅ Detecta posse da bola via weld
    local function setupBallWatcher()
        local function checkBall()
            ...
            -- Observa quando bola for pega/largada
    ...
    local function applyVelocity()
        ...
        bv.Velocity = horizontalDir * getCurrentTarget()  -- ✅ Target dinâmico
    end
...
run(function()
    ...
    -- Posições EXATAS que você capturou
    ...
    AutoGoal = vape.Categories.realista:CreateModule({
        Name = 'AutoGoal',
        Function = function(callback)
            if callback then
                if not entitylib.isAlive then
                    notif('AutoGoal', 'Você precisa estar vivo.', 3, 'warning')
                    AutoGoal:Toggle()
                    return
                end

                if not ShootRemote then
                    notif('AutoGoal', 'Remote de chute não encontrado.', 5, 'warning')
                    AutoGoal:Toggle()
                    return
                end
                ...
                local ball = findBall()
                if not ball then
                    notif('AutoGoal', 'Bola não encontrada.', 3, 'warning')
                    AutoGoal:Toggle()
                    return
                end
                ...
                -- 2. Decide qual trave usar (oposta ao goleiro)
...
run(function()
    ...
    AutoFarmer = vape.Categories.realista:CreateModule({
        Name = 'AutoFarmer',
        Function = function(callback)
            if callback then
                stats = {
...
run(function()
    ...
    local DRIBBLE_ANIM_IDS = {
        ['14328586766'] = 'Spin',
        ['14893581487'] = 'Jump Deke',
        ['14899049391'] = 'Rainbow Flick',
        ['14908212300'] = 'StepOver',
        ['15154280353'] = 'Ronaldo Chop',
        ['16331006011'] = 'Elastico',
        ['17590081108'] = 'Sombrero Flick',
        ['17590392784'] = 'Ball Hop',
        ['17743274227'] = 'Cruyff Turn',
        ['17748025748'] = 'Antony Spin',
        ['121059135011282'] = 'Twist Flip',
        ['140030983315202'] = 'La Croqueta2',
        ['133996236884225'] = 'La Croqueta1',
        ['127593117332707'] = 'Drag Back',
        ['107226443405165'] = 'Around The World'
    }
    ...
    local function findEnemyWithBall()
        ...
        if owner == lplr then return nil end
        if owner.Team == lplr.Team then return nil end
        return owner
    end
    ...
    local function isEnemyVulnerable(enemy)
        ...
        if lastDribble then
            ...
            if timeSince > 0.5 and timeSince < (DRIBBLE_COOLDOWN - 1) then
                return true
            end
        end
        ...
        if bools then
            ...
            if grabTick and grabTick.Value > 0 then
                local timeSinceGrab = tick() - grabTick.Value
                if timeSinceGrab < 0.3 then return true end
            end
        end
    ...
    local function doTackle()
        ...
        local bv = Instance.new("BodyVelocity")
        bv.Parent = root
        bv.velocity = root.CFrame.lookVector * TackleSpeed.Value  -- ✅ usa slider
    ...
    local function aimAtTarget(target, dist)
        if not entitylib.isAlive or not target or not target.Character then return end

        local enemyRoot = target.Character:FindFirstChild('HumanoidRootPart')
        if not enemyRoot then return end

        local myRoot = entitylib.character.RootPart
        local timeToReach = dist / TackleSpeed.Value  -- ✅ usa slider
        local enemyVel = enemyRoot.AssemblyLinearVelocity
        local predictedPos = enemyRoot.Position + (enemyVel * timeToReach)
    ...
    local function executeAttack(enemy, dist)
    ...
    local function eagleEyeMode()
        ...
        local myRoot = entitylib.character.RootPart
        if not enemyRoot or not myRoot then return end

        local dist = (enemyRoot.Position - myRoot.Position).Magnitude
        if dist > DetectRange.Value then return end

        executeAttack(enemy, dist)
    end

    local function onlyDribbleMode()
        if isAttacking then return end
        if not canTackle() then return end
        ...
        if not isEnemyVulnerable(enemy) then return end
    ...
    local function manualTackleMode()
        if not canTackle() then
            notif('AutoTackle', 'Tackle em cooldown.', 2, 'warning')
            AutoTackle:Toggle()
            return
        end

        if not entitylib.isAlive then
            notif('AutoTackle', 'Você precisa estar vivo.', 3, 'warning')
            AutoTackle:Toggle()
            return
        end
        ...
        local enemy = findEnemyWithBall()
        if not enemy then
            notif('AutoTackle', 'Nenhum inimigo com bola encontrado.', 3, 'warning')
            AutoTackle:Toggle()
            return
        end
        ...
        if dist > DetectRange.Value then
            notif('AutoTackle', 'Inimigo fora do range. (' .. math.floor(dist) .. ' studs)', 3, 'warning')
            AutoTackle:Toggle()
            return
        end
    ...
    local function isDribbleAnim(animId)
        if not animId then return false end
        for id, _ in pairs(DRIBBLE_ANIM_IDS) do
            if animId:find(id) then return true end
        end
        return false
    end
    ...
    AutoTackle = vape.Categories.realista:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not ActionRemote then
                    notif('AutoTackle', 'Remote Action não encontrado.', 5, 'warning')
                    AutoTackle:Toggle()
                    return
                end
                ...
                AutoTackle:Clean(game.Players.PlayerAdded:Connect(watchPlayerDribbles))

                AutoTackle:Clean(runService.Heartbeat:Connect(function()
                    if not entitylib.isAlive then return end

                    if Mode.Value == 'Eagle Eye' then
                        eagleEyeMode()
                    elseif Mode.Value == 'Only Dribble' then
                        onlyDribbleMode()
                    elseif Mode.Value == 'Smart' then
                        smartMode()
                    end
                end))
            else
        ...
        Tooltip = 'Auto tackle com slider de velocidade.'
    })

    Mode = AutoTackle:CreateDropdown({
        Name = 'Mode',
        List = {'Eagle Eye', 'Only Dribble', 'Smart', 'Manual Tackle'},
        Tooltip = 'Eagle Eye: sempre | Only Dribble: só quando vulnerável | Smart: combina | Manual: executa uma vez'
    })
    ...
    TackleSpeed = AutoTackle:CreateSlider({
        ...
        Tooltip = '4v4 = 40 | 7v7 = 50 | Maior = mais forte (mas pode ser rejeitado)'
    })
end)
...
run(function()
    ...
    local PickupRemote
    for _, v in game:GetService('ReplicatedStorage'):WaitForChild('Remotes'):GetChildren() do
    ...
    local function getBallOwner()
        ...
        local rootPart = weld.Part0
        if not rootPart then return nil end
        return game.Players:GetPlayerFromCharacter(rootPart.Parent)
    end
        ...
        Function = function(callback)
            if callback then
                if not PickupRemote or not ShootRemote then
                    notif('AerialReach', 'Remotes não encontrados.', 5, 'warning')
                    AerialReach:Toggle()
                    return
                end
                ...
                AerialReach:Clean(runService.Heartbeat:Connect(function()
                    ...
                    if tick() - lastShot < 3 then return end
                    ...
                    if ball.Position.Y < MinHeight.Value then return end
                    ...
                    notif('AerialReach', string.format('Bicicleta! Dist: %d ✈️', math.floor(dist)), 2)
                end))
            end
        end,
        Tooltip = 'Reach aéreo simples com altura mínima e timeout configuráveis.'
    })
    ...
    Delay = AerialReach:CreateSlider({
        ...
        Tooltip = 'Tempo entre apertar ESPAÇO e o reach ativar'
    })

    MinHeight = AerialReach:CreateSlider({
        Name = 'Min Ball Height',
        Min = 6.5, Max = 30, Default = 8, Decimal = 10,
        Suffix = function(val) return val == 1 and 'stud' or 'studs' end,
        Tooltip = 'Altura mínima da bola pra disparar o chute'
    })
    ...
    Timeout = AerialReach:CreateSlider({
        ...
        Suffix = function(val) return val == 1 and 'second' or 'seconds' end,
        Tooltip = 'Tempo máximo esperando a bola entrar no range antes de desistir'
    })
end)
...
run(function()
    ...
    local shootAnim = Animations:FindFirstChild('RShoot2') or Animations:FindFirstChild('RShoot')
    ...
    local function findGoalie(goalPos)
        ...
        if bot and bot:FindFirstChild('HumanoidRootPart') then
            local dist = (bot.HumanoidRootPart.Position - goalPos).Magnitude
            if dist < 60 then
        ...
        for _, plr in game.Players:GetPlayers() do
            if plr ~= lplr and plr.Team ~= lplr.Team 
               and plr.Character and plr.Character:FindFirstChild('HumanoidRootPart') then
                local pos = plr.Character.HumanoidRootPart.Position
                local dist = (pos - goalPos).Magnitude
    ...
    local function getBestTarget(goal)
        ...
        if not targets then return nil, nil, nil, nil end
        ...
        local gkSide = goalie.Position.Z > goalCenterPos.Z and "right" or "left"
        ...
        local target = distRight > distLeft and targetRight or targetLeft
        local oppositeTarget = distRight > distLeft and targetLeft or targetRight
        
        return target, gkSide, isBot, oppositeTarget
    end
    ...
    local function getCurveDirection(gkSide, isBot)
        ...
        if PreferSpin.Enabled and isBot then
            return "None"
        end
        
        if gkSide == "right" then
            return "Left"
        elseif gkSide == "left" then
            return "Right"
        else
            return "None"
        end
    ...
    AutoShoot = vape.Categories.realista:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not entitylib.isAlive then
                    notif('AutoShoot', 'Você precisa estar vivo.', 3, 'warning')
                    AutoShoot:Toggle()
                    return
                end
                ...
                if not haveBallPossession() then
                    notif('AutoShoot', 'Você não está com a bola!', 3, 'warning')
                    AutoShoot:Toggle()
                    return
                end
                ...
                local ballInChar = char:FindFirstChild("ball")
                if not ballInChar then ballInChar = findBall() end
                if not ballInChar then
                    AutoShoot:Toggle()
                    return
                end
                ...
                if not goal then
                    notif('AutoShoot', 'Gol não encontrado!', 3, 'warning')
                    AutoShoot:Toggle()
                    return
                end
                ...
                if not target then
                    notif('AutoShoot', 'Target não encontrado!', 3, 'warning')
                    AutoShoot:Toggle()
                    return
                end

                local distToGoal = (ballInChar.Position - target.Position).Magnitude
                if distToGoal > MaxRange.Value then
                    notif('AutoShoot', 'Muito longe! (' .. math.floor(distToGoal) .. ')', 3, 'warning')
                    AutoShoot:Toggle()
                    return
                end

                -- ✅ DRAG
                local heightCompensation = 0
                if distToGoal > DragFreeZone.Value then
                    local extraDist = distToGoal - DragFreeZone.Value
                    local v = Power.Value * BALL_SPEED_PER_POWER
                    local t = extraDist / v
                    heightCompensation = 0.5 * GRAVITY * t * t * Drag.Value
                end

                -- ✅ CURVA (SEM limite de distância)
                local curveValue = "None"
                local shootMode = "reto"
                
                if DynamicShoot.Enabled then
                    local goalie = findGoalie(
                        (goal.Targets.TargetRight.Position + goal.Targets.TargetLeft.Position) / 2
                    )
                    
                    if goalie and isGKCentered(goal, goalie) then
                        curveValue = getCurveDirection(gkSide, isBot)
                        if curveValue ~= "None" then
                            shootMode = "curva (GK centro)"
                        end
                    else
                        curveValue = "None"
                        shootMode = "reto (GK lado)"
                    end
                else
                    curveValue = getCurveDirection(gkSide, isBot)
                    if curveValue ~= "None" then
                ...
                -- ✅ DERIVATION MULT
                local aimTarget = target.Position + Vector3.new(0, heightCompensation, 0)
                
                if curveValue ~= "None" and oppositeTarget and DerivationMult.Value > 0 then
                    local factor = DerivationMult.Value / 10
                    local targetPos = target.Position + Vector3.new(0, heightCompensation, 0)
                    local oppositePos = oppositeTarget.Position + Vector3.new(0, heightCompensation, 0)
                    aimTarget = targetPos:Lerp(oppositePos, factor)
                end

                local shootDir = (aimTarget - ballInChar.Position).Unit
                local hitLookVector = shootDir

                local charBools = char:FindFirstChild("Bools")
                if charBools then
                    local curve = charBools:FindFirstChild("Curve")
                    if curve then
                        curve.Value = curveValue
                    end
                end

                if shootAnim and hum then
                    local track = hum:LoadAnimation(shootAnim)
                ...
                local gkType = isBot and "Bot" or "Player"
                local curveMsg = curveValue ~= "None" and " | Curve " .. curveValue .. " D:" .. DerivationMult.Value or ""
                notif('AutoShoot', string.format('%s | %d studs | %s | GK: %s (%s)%s', 
                    target.Name, 
                    math.floor(distToGoal),
                    shootMode,
                    gkSide or "?",
                    gkType,
                    curveMsg
        ...
        Tooltip = 'AutoShoot com DynamicShoot + curva sem limite.'
    })
...
run(function()
    ...
    FarmPro = vape.Categories.realista:CreateModule({
        Name = 'FarmPro',
        Function = function(callback)
            if callback then
                stats = {
...
run(function()
    ...
    local function getTargetGoal()
        local myColor = lplr.TeamColor and lplr.TeamColor.Number
        if myColor == HOME_TEAM_COLOR then return workspace:FindFirstChild("HomeGoal")
        elseif myColor == AWAY_TEAM_COLOR then return workspace:FindFirstChild("AwayGoal") end
        return workspace:FindFirstChild("HomeGoal")
    end
    ...
    local function getBestTarget(goal)
        ...
        if not targetRight or not targetLeft then return targetCenter, targetCenter end

        local goalCenterPos = targetCenter and targetCenter.Position or 
                              (targetRight.Position + targetLeft.Position) / 2
        
        local goalie = findGoalie(goalCenterPos)
        if not goalie then return targetRight, targetLeft end
    ...
    AerialReach = vape.Categories.realista:CreateModule({
            ...
            if callback then
                ...
                AerialReach:Clean(runService.Heartbeat:Connect(function()
                    ...
                    if AutoAim.Enabled then
                        local goal = getTargetGoal()
                        if goal then
                            local goalTargets = goal:FindFirstChild("Targets")
                            local goalCenter = goalTargets and goalTargets:FindFirstChild("TargetCenter")
                            
                            if goalCenter then
                                local distToGoal = (root.Position - goalCenter.Position).Magnitude
                                
                                if distToGoal <= AimRange.Value then
                                    local target, oppositeTarget = getBestTarget(goal)
                                    if target then
                                        local aimPos = target.Position
                                        
                                        if oppositeTarget and AerialDerivation.Value > 0 then
                                            local factor = AerialDerivation.Value / 10
                                            aimPos = target.Position:Lerp(oppositeTarget.Position, factor)
                                        end
                                        
                                        local horizDir = Vector3.new(
                                            aimPos.X - ball.Position.X,
                                            0,
                                            aimPos.Z - ball.Position.Z
                                        ).Unit
                                        
                                        local originalY = root.CFrame.LookVector.Y
                                        shootDir = Vector3.new(horizDir.X, originalY, horizDir.Z).Unit
        ...
        Tooltip = 'Reach aéreo com altura mínima e timeout configuráveis.'
    })

    Range = AerialReach:CreateSlider({
        Name = 'Range', Min = 5, Max = 50, Default = 15,
        Suffix = function(val) return val == 1 and 'stud' or 'studs' end
    })

    Delay = AerialReach:CreateSlider({
        Name = 'Delay', Min = 0, Max = 2, Default = 0.3, Decimal = 100,
        Suffix = function(val) return val == 1 and 'second' or 'seconds' end
    })
    ...
    AutoAim = AerialReach:CreateToggle({
        Name = 'Auto Aim', Default = true,
        Function = function(callback)
            if AimRange and AimRange.Object then AimRange.Object.Visible = callback end
            if AerialDerivation and AerialDerivation.Object then AerialDerivation.Object.Visible = callback end
            if TurnBody and TurnBody.Object then TurnBody.Object.Visible = callback end
        end,
        Tooltip = 'Mira horizontal no canto oposto ao GK'
    })

    AimRange = AerialReach:CreateSlider({
        Name = 'Aim Range', Min = 10, Max = 200, Default = 80,
        Visible = false,
        Suffix = function(val) return val == 1 and 'stud' or 'studs' end
    })

    AerialDerivation = AerialReach:CreateSlider({
        Name = 'Derivation Mult', Min = 0, Max = 10, Default = 0, Decimal = 10,
        Visible = false,
        Tooltip = '0=reto no canto | 10=mira no lado oposto'
    })
    ...
    TurnBody = AerialReach:CreateToggle({
        ...
        Default = true,
        Visible = false,
        Tooltip = 'Vira o corpo na direção do alvo (complemento visual)'
    })
end)
...
run(function()
    ...
    local function getBallOwner()
        ...
        local weld = ball:FindFirstChild("playerWeld")
        if not weld or not weld:IsA("Weld") then return nil end
        return game.Players:GetPlayerFromCharacter(weld.Part0.Parent)
    end
        ...
        Function = function(callback)
            if callback then
                if not PickupRemote then
                    notif('BallMagnet', 'Remote de pickup não encontrado.', 5, 'warning')
                    BallMagnet:Toggle()
                    return
                end
                ...
                BallMagnet:Clean(runService.Heartbeat:Connect(function()
                    if not entitylib.isAlive then return end
                    if tick() - lastPickup < MagnetDelay.Value then return end
                    ...
                    local boolsFolder = workspace:FindFirstChild("Bools")
                    if boolsFolder then
                        local cantGrab = boolsFolder:FindFirstChild("cantGrab")
                        local penalty = boolsFolder:FindFirstChild("Penalty")
                        if (cantGrab and cantGrab.Value) or (penalty and penalty.Value) then
                            return
                        end
                    end
                    ...
                    local horizontalDist = (Vector3.new(myPos.X, 0, myPos.Z) - Vector3.new(ballPos.X, 0, ballPos.Z)).Magnitude
                    if horizontalDist > RangeX.Value then return end

                    local upwardOffset = ballPos.Y - myPos.Y

                    if RangeY.Value == 0 then
                        if upwardOffset > 0 then return end
                    else
                        if upwardOffset > RangeY.Value then return end
                    end

                    lastPickup = tick()
                    -- ✅ Mente distância 0 pro servidor (prioridade máxima)
                    PickupRemote:FireServer(0)
                end))
            end
        end,
        Tooltip = 'Pickup configurável. Dispara com distância 0 pra prioridade máxima.'
    })
    ...
    MagnetDelay = BallMagnet:CreateSlider({
        ...
        Suffix = function(val) return val == 1 and 'second' or 'seconds' end,
        Tooltip = 'Tempo entre cada tentativa de pickup'
    })
end)
